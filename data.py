...
teachers = [
...

    {
        "id": 12,
        "name": "Julie Nguyen",
        "about": "Сертифицированный репетитор IELTS."
                 " Являюсь сертифицировнным CELTA преподавателем английского "
                 "языка, проживающим за границей."
                 " Опыт работы в сфере преподавания - 9 лет.",
        "picture": "https://i.pravatar.cc/300?img=38",
        "rating": 4.8,
        "price": 900,
...

    {
        "id": 13,
        "name": "Wendy Edwards",
        "about": "Молодой и современный репетитор и переводчик"
                 " с четырёхлетним опытом работы репетиторства Индивидуальные"
                 "занятия по скайпу, которые ведёт"
                 " молодой специалист отделения 'Перевода'",
        "picture": "https://i.pravatar.cc/300?img=38",
        "rating": 4.8,
        "price": 1300,
        "goals": ["travel", "work", "relocate"],
        "free": {
...
with open("goals.json", "w", encoding="utf-8") as db:
    json.dump(
        goals,
        db,
        ensure_ascii=False,
    ...
    db.write("\n")
with open("teachers.json", "w", encoding="utf-8") as db:
    json.dump(
        teachers,
        db,
        ensure_ascii=False,
